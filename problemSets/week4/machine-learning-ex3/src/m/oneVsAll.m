...
for class_index = 1:num_labels
   ...
   % `y_bin[i]` is 1 iff the ith training example is an element of
   % `class_index` (i.e., `y[i]` equals `class_index`); otherwise
   % `y_bin[i]` is 0.
   y_bin = (y == class_index);
   [theta] = fmincg(@(theta) (lrCostFunction(theta, X, y_bin, ...
                                             lambda)), ...
                    theta_init, ...
                    options);
